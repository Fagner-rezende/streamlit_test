...
# setando a pagina para wide
st.set_page_config(layout='wide')


# função para formatar os números para melhor exibição
...
# para colocar um título na página
st.title('DASHBOARD DE VENDAS :shopping_trolley:')

# para acessar a url da nossa api
url = 'https://labdados.com/produtos'
regioes = ['Brasil', 'Centro-Oeste', 'Nordesde', 'Norte', 'Sudeste', 'Sul']
...
todos_anos = st.sidebar.checkbox('Dados de todo o préiodo', value = True)
if todos_anos:
    ano = ''
else:
...
# transformando os dados em um json e depois em df
dados = pd.DataFrame.from_dict(response.json())
dados['Data da Compra'] = pd.to_datetime(
    dados['Data da Compra'], format='%d/%m/%Y')

filto_vendedores = st.sidebar.multiselect('Vendedores', dados['Vendedor'].unique())
if filto_vendedores:
    dados = dados[dados['Vendedor'].isin(filto_vendedores)]
...
with aba1:
    ...
    with coluna2:
        # para pegar a quantidade de linhas shape
